...
Lobby.prototype.initialize = function() {
  ...
  this.viewPreviousResultsButton.click(function() {
    self.socket.emit("viewPreviousResults", {});
  });
...
Lobby.prototype.update = function(res) {
  if (res.success) {
    ...
    if (res.player.isAdmin) {
      ...
      this.startButton.removeClass(constants.HIDDEN);
      //show the game Settings
      this.gameSettings.removeClass(constants.HIDDEN);
    } else {
      this.startButton.addClass(constants.HIDDEN);
      this.gameSettings.addClass(constants.HIDDEN);
    }

    if (res.data.canViewLastRoundResults) {
      this.viewPreviousResultsButton.removeClass(constants.HIDDEN);
    } else {
      this.viewPreviousResultsButton.addClass(constants.HIDDEN);
    }
  } else {
...
Lobby.prototype.start = function() {
  ...
  let wordPack = $(`${this.wordPackDropdown.selector} option:selected`).text();
